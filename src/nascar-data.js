...
const SEASON = Number(ENV.SEASON || new Date().getFullYear());
  ...
  return cacheGet("season-stenhouse-results", 30_000, async () => {
    ...
    const races = schedule
      ...
      .filter((race) => {
        ...
        return race.inspection_complete || Number.isFinite(start) && start <= Date.now();
      });
...
export async function getScheduleDashboard() {
  ...
  const futureRaces = schedule
    ...
    .filter((race) => {
      ...
      return Number.isFinite(start) && start > now && !race.inspection_complete;
    })
...
function selectRace(races) {
  ...
  const activeWindow = sorted.find((race) => {
    ...
    return !race.inspection_complete
...
function isCompleteRaceFeed(weekendRace, liveFeed) {
  if (weekendRace?.inspection_complete || weekendRace?.winner_driver_id) return true;
  ...
  const raceLaps = Number(liveFeed.laps_in_race || weekendRace?.actual_laps || weekendRace?.scheduled_laps);
...
function buildStagePlan(weekendRace, liveFeed, currentLap, current) {
  const scheduledLaps = numberOrZero(liveFeed?.laps_in_race || weekendRace?.scheduled_laps);
  ...
  const currentStage = liveStageNumber || stages.find((stage) => lap <= stage.endLap)?.stageNumber || stages.at(-1).stageNumber;
...
function normalizeRace(race, weekendFeed, liveFeed, errors) {
  ...
  const scheduledLaps = liveFeed?.laps_in_race || weekendRace.scheduled_laps || "";
  ...
  return {
    ...
    scheduledLaps,
    currentLap,
    lapsToGo: status === "complete" ? 0 : liveFeed?.laps_to_go || "",

...
const MELON_HERO = [
  "radial-gradient(circle at 18% 24%, rgba(77, 201, 93, 0.22), transparent 20rem)",
  "radial-gradient(circle at 82% 72%, rgba(255, 82, 96, 0.2), transparent 22rem)",
  "linear-gradient(128deg, rgba(8, 9, 9, 0.94) 0%, rgba(19, 26, 19, 0.86) 44%, rgba(58, 11, 20, 0.88) 100%)"
].join(",");
...
async function loadDashboard() {
  ...
  try {
    const response = await fetch("/api/melon-patch", { cache: "no-store" });
    if (!response.ok) {
...
function render(data) {
  ...
  document.documentElement.style.setProperty("--hero-image", MELON_HERO);
  fields.hyakLogo.src = assets.hyakLogo;
  fields.hyakLogo.hidden = true;

  fields.raceStatus.textContent = currentRace.status === "live" ? "Live now" : currentRace.status;
  fields.heroSponsor.textContent = `${value(stenhouse.sponsor, driver.sponsor)} / Watermelon mode`;
...
function stageMemoryKey(raceId) {
  return `melon-patch-stage-memory:${raceId}`;
}
...
function renderRaceHighlights(currentRace, stages, memory) {
  ...
  const stats = [
    ...
    ["Laps Led", value(stenhouse.lapsLed, "0"), "No. 1 out front"],
...
function renderLeaderboard(currentRace) {
  ...
  fields.leaderboardCount.textContent = stenhouse && !rows.some((row) => row.isStenhouse) ? "Top 10 + 1" : "Top 10";

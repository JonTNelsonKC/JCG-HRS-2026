...
function updateRaceCountdown() {
  ...
  if (currentRace?.status === "live" || currentRace?.status === "complete") {
    fields.raceCountdown.textContent = "";
    fields.raceCountdown.dataset.state = currentRace.status;
    showFlagSlot();
    return;
  }
  ...
  if (currentRace.status === "live") {
    fields.raceCountdown.textContent = "Live now";
    fields.raceCountdown.dataset.state = "live";
    return;
  }

  if (currentRace.status === "complete") {
    fields.raceCountdown.textContent = "Complete";
    fields.raceCountdown.dataset.state = "complete";
    return;
  }

  const start = new Date(currentRace.scheduled).getTime();
  ...
  fields.raceCountdown.dataset.state = diffMs <= 0 ? "live" : "pending";
}

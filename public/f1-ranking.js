...
const FAN_RANKINGS_API = "/api/f1-rankings";
...
async function rankingsApiRequest(method, body) {
  const response = await fetch(FAN_RANKINGS_API, {
    method,
    headers: {
...
async function loadSharedEntries() {
  return rankingsApiRequest("GET");
}

async function saveSharedEntry(entry) {
  return rankingsApiRequest("POST", entry);
}

async function deleteSharedEntry(entry) {
  return rankingsApiRequest("DELETE", entry);
}

function isOnlineStoreMissing(error) {
  return /online store not configured/i.test(String(error?.message || ""));
}
...
async function setupFanRanking() {
  ...
  const syncSavedEntries = (entries) => {
    savedEntries = Array.isArray(entries) ? entries.slice(0, MAX_FAN_RANKINGS) : [];
    writeSavedEntries(savedEntries);
    renderSavedRankings(savedEntries, dropSavedEntry);
  };

  const dropSavedEntry = async (entry) => {
    ...
    try {
      const remoteEntries = await deleteSharedEntry({
        id: entry.id,
        pin: String(enteredPin).trim()
      });
      syncSavedEntries(remoteEntries);
      fanStatus(`Deleted ${entry.name}'s ranking.`);
    } catch (error) {
      if (isOnlineStoreMissing(error)) {
        fanStatus("Online storage is not configured yet. Add KV binding F1_RANKINGS in Cloudflare.", "error");
        return;
      }
      fanStatus(error.message || "Could not delete ranking right now.", "error");
    }
  };
  ...
  try {
    const remoteEntries = await loadSharedEntries();
    syncSavedEntries(remoteEntries);
    fanStatus("Shared rankings synced.");
  } catch (error) {
    if (isOnlineStoreMissing(error)) {
      fanStatus("Online storage is not configured yet. Add KV binding F1_RANKINGS in Cloudflare.", "error");
      return;
    }
    fanStatus("Shared rankings unavailable. Saving to this browser only.", "error");
  }
  ...
  fanFields.form.addEventListener("submit", async (event) => {
    event.preventDefault();
    ...
    try {
      const remoteEntries = await saveSharedEntry({
        name: trimmedName,
        ranking
      });
      syncSavedEntries(remoteEntries);
      fanStatus(`${existingEntry ? "Updated" : "Saved"} ${trimmedName}'s ranking.`);
    } catch (error) {
      if (isOnlineStoreMissing(error)) {
        fanStatus("Online storage is not configured yet. Add KV binding F1_RANKINGS in Cloudflare.", "error");
        return;
      ...
      syncSavedEntries([entry, ...withoutName].slice(0, MAX_FAN_RANKINGS));
      fanStatus(`Saved ${entry.name} locally. Shared sync unavailable.`, "error");
    }
  });
}

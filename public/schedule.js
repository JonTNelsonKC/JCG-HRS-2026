  ...
  return `
    <article class="schedule-card">
      ...
      <div>
        ...
        <span class="schedule-card__type schedule-card__type--${race.isPointsRace ? "points" : "special"}">${value(race.raceTypeLabel, "Cup Event")}</span>
      </div>
...
function render(data) {
  const hasSpecialEvents = data.races.some((race) => race.isPointsRace === false);
  fields.count.textContent = `${data.races.length} future ${hasSpecialEvents ? "events" : "races"}`;
  fields.season.textContent = `${data.season} Cup season`;
  fields.grid.innerHTML = data.races.length
    ? data.races.map(raceCard).join("")
    : `<div class="empty-state">No future Cup events found in the current NASCAR schedule feed.</div>`;

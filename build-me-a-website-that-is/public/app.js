...
function renderRaceHighlights(currentRace, stages, memory) {
  ...
  fields.stageRange.textContent = currentRace.currentLap
    ? `Finished lap ${currentRace.currentLap}/${value(currentRace.scheduledLaps)}`
    : `Race length ${value(currentRace.scheduledLaps)} laps`;
...
function renderStageTracker(currentRace) {
  ...
  fields.stageRange.textContent = plan.currentStageEndLap
    ? `Laps ${value(plan.currentStageStartLap, "1")}-${plan.currentStageEndLap}`
    : `Race length ${value(currentRace.scheduledLaps)} laps`;

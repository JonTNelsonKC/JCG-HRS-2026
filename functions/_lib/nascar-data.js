...
function defaultSeasonYear() {
  const runtimeYear = currentYear();
  if (!Number.isFinite(runtimeYear) || runtimeYear < MIN_SEASON_YEAR) {
...
function getSeason() {
  const runtimeYear = defaultSeasonYear();
  const maxSeasonYear = runtimeYear + MAX_SEASON_AHEAD;
  const raw = globalThis.process?.env?.NASCAR_SEASON;
  if (raw === undefined || raw === null || raw === "") return runtimeYear;

  const parsed = Number.parseInt(String(raw), 10);
  if (!Number.isFinite(parsed)) return runtimeYear;
  if (parsed < MIN_SEASON_YEAR || parsed > maxSeasonYear) return runtimeYear;
  return parsed;
}
...
const STAT_ROLLOVER_TIME_ZONE = globalThis.process?.env?.STAT_ROLLOVER_TIME_ZONE || "America/New_York";
const WEEKDAY_INDEX = { Sun: 0, Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6 };
const FEATURED_CUP_RACE_TYPES = new Set([1, 2]);
...
const ROSS_FALLBACK = {
  ...
  driver: {
    ...
    manufacturer: "Chevrolet",
    crewChief: "Brandon McSwain",
    hometown: "Alva, Florida",
    dateOfBirth: "12/4/1992",
    sponsor: "Busch Light",
    officialPage: "https://www.nascar.com/drivers/ross-chastain",
    teamPage: "https://www.trackhouse.com/nascar#Chastain"
  },
  season: {
    year: FALLBACK_SEASON_YEAR,
    rank: "24",
  ...
  raceLog: [
    { track: "Bowman Gray Stadium", start: "9", finish: "9", points: "0" },
    { track: "Daytona International Speedway", start: "18", finish: "21", points: "0" },
    { track: "Daytona International Speedway", start: "37", finish: "20", points: "17" },
    { track: "Atlanta Motor Speedway", start: "21", finish: "3", points: "34" },
    { track: "Circuit of The Americas", start: "2", finish: "35", points: "13" },
    { track: "Phoenix Raceway", start: "6", finish: "28", points: "10" },
    { track: "Las Vegas Motor Speedway", start: "17", finish: "17", points: "20" },
    { track: "Darlington Raceway", start: "25", finish: "16", points: "21" },
    { track: "Martinsville Speedway", start: "18", finish: "16", points: "21" },
    { track: "Bristol Motor Speedway", start: "6", finish: "20", points: "17" },
    { track: "Talladega Superspeedway", start: "24", finish: "7", points: "41" }
  ],
  annualHistory: [
    { year: "2026", rank: "24", wins: "0", top5: "1", top10: "2", poles: "0", lapsLed: "60", avgStart: "20.07", avgFinish: "21.93" },
    { year: "2025", rank: "10", wins: "1", top5: "4", top10: "12", poles: "0", lapsLed: "83", avgStart: "19.03", avgFinish: "15.81" },
    { year: "2024", rank: "19", wins: "1", top5: "6", top10: "14", poles: "1", lapsLed: "311", avgStart: "15.81", avgFinish: "14.94" },
    { year: "2023", rank: "9", wins: "2", top5: "10", top10: "14", poles: "1", lapsLed: "640", avgStart: "15.58", avgFinish: "15.00" },
    { year: "2022", rank: "2", wins: "2", top5: "15", top10: "21", poles: "0", lapsLed: "692", avgStart: "14.42", avgFinish: "13.25" }
  ],
  careerHighlights: [
    {
      category: "Cup Win",
      year: "2022",
      title: "First Cup win at Circuit of The Americas",
      detail: "Delivered both his first Cup victory and the first Trackhouse Racing Cup win at COTA."
    },
    {
      category: "Cup Win",
      year: "2022",
      title: "Talladega follow-up",
      detail: "Backed up the COTA breakthrough four weeks later with another Cup win at Talladega."
    },
    {
      category: "Playoff Moment",
      year: "2022",
      title: "Martinsville wall ride",
      detail: "Pulled off the last-lap Martinsville move that sent the No. 1 to the Championship 4."
    },
    {
      category: "Crown Jewel",
      year: "2025",
      title: "Coca-Cola 600 winner",
      detail: "Delivered Trackhouse Racing its first crown-jewel victory at Charlotte Motor Speedway."
    },
    {
      category: "Truck Series",
      year: "2019",
      title: "Three-win title push",
      detail: "Shifted midseason into a serious Truck title run and stacked three victories with Niece Motorsports."
    }
  ]
};
...
const RICKY_DRIVER = {
  id: DRIVER_ID,
  name: DRIVER_NAME,
  number: FALLBACK.driver.number,
  slug: "ricky-stenhouse-jr",
  fallback: FALLBACK
};

const ROSS_DRIVER = {
  id: "",
  name: ROSS_FALLBACK.driver.name,
  number: ROSS_FALLBACK.driver.number,
  slug: "ross-chastain",
  fallback: ROSS_FALLBACK
};
...
function baseSources(season = getSeason(), featuredDriver = RICKY_DRIVER) {
  const driver = featuredDriver.fallback.driver;
  return [
    {
      label: "NASCAR driver stats",
      url: driver.officialPage
    },
    {
    ...
    {
      label: featuredDriver === ROSS_DRIVER ? "Trackhouse Racing" : "HYAK Motorsports",
      url: driver.teamPage
    }
  ];
}
...
function dashboardSources(currentRace, season = getSeason(), featuredDriver = RICKY_DRIVER) {
  ...
  return [
    ...baseSources(season, featuredDriver),
    ...raceSources,
    ...SECONDARY_SOURCES
  ];
}
...
function isPointsRace(raceLike) {
  return Number(raceLike?.race_type_id) === 1;
}

function isFeaturedCupRace(raceLike) {
  return FEATURED_CUP_RACE_TYPES.has(Number(raceLike?.race_type_id));
}

function raceTypeLabel(raceTypeId) {
  const numeric = Number(raceTypeId);
  if (numeric === 1) return "Points Race";
  if (numeric === 2) return "All-Star Event";
...
async function getScheduleForSeason(season = getSeason()) {
...
async function getSchedule() {
  return getScheduleForSeason(getSeason());
}
...
async function getWeekendFeed(raceId) {
  return getWeekendFeedForSeason(getSeason(), raceId);
}
...
async function getPreviousTrackResults(trackId, trackName) {
  const previousSeason = getSeason() - 1;
...
export async function getScheduleDashboard() {
  ...
  const races = await Promise.all(futureRaces.map(async (race) => {
    ...
    try {
      ...
      warnings.push(`${race.track_name} ${previousSeason} lookup: ${error.message}`);
    }
    ...
    return {
      ...
      raceTypeId: Number(race.race_type_id || 0),
      raceTypeLabel: raceTypeLabel(race.race_type_id),
      isPointsRace: isPointsRace(race),
  ...
  return {
    ...
    sources: [
      ...
      },
      {
        ...
        url: `https://cf.nascar.com/cacher/${previousSeason}/race_list_basic.json`
      },
      {
...
async function getDriverMarkdown(featuredDriver = RICKY_DRIVER) {
  return cacheGet(`driver-markdown:${featuredDriver.slug}`, 60_000, async () => {
    const readerUrl = `https://r.jina.ai/http://https://www.nascar.com/drivers/${featuredDriver.slug}`;
...
function parseDriverMarkdown(markdown, fallbackData = FALLBACK) {
  const seasonYear = getSeason();
  const lines = cleanLines(markdown);
  const summaryStart = lines.findIndex((line) => line.startsWith(`${seasonYear} SEASON`));
  ...
  const driver = {
    ...fallbackData.driver,
    dateOfBirth: metricFromLines(lines, "DATE OF BIRTH") || fallbackData.driver.dateOfBirth,
    hometown: metricFromLines(lines, "HOMETOWN") || fallbackData.driver.hometown,
    crewChief: metricFromLines(lines, "CREW CHIEF") || fallbackData.driver.crewChief,
    team: metricFromLines(lines, "TEAM") || fallbackData.driver.team
  };

  return {
    driver,
    season: { ...fallbackData.season, ...blankless(season) },
    raceLog: parseRaceLog(lines, fallbackData.raceLog),
    annualHistory: parseAnnualHistory(lines, fallbackData.annualHistory),
    careerHighlights: fallbackData.careerHighlights
  };
}
...
function parseRaceLog(lines, fallbackRaceLog = FALLBACK.raceLog) {
  ...
  return rows.length ? rows : fallbackRaceLog;
}

function parseAnnualHistory(lines, fallbackAnnualHistory = FALLBACK.annualHistory) {
  const yearIndex = lines.findIndex((line) => line === "YEAR");
  if (yearIndex === -1) return fallbackAnnualHistory;

  const headerEnd = lines.findIndex((line, index) => index > yearIndex && line === "AVG. FINISH");
  if (headerEnd === -1) return fallbackAnnualHistory;
  ...
  return rows.length ? rows.slice(0, 14) : fallbackAnnualHistory;
}
...
function nextRace(races) {
  ...
  return races
    ...
    .map((race) => ({
      race,
      start: new Date(raceTime(race)).getTime()
    }))
    .filter(({ start }) => Number.isFinite(start) && start > now)
    .sort((a, b) => a.start - b.start)[0]?.race || null;
}

function driverMatches(entry, featuredDriver = RICKY_DRIVER) {
  const id = entry.driver_id || entry.driver?.driver_id;
  const name = String(entry.driver_fullname || entry.driver_name || entry.driver?.full_name || "").toLowerCase();
  const number = String(entry.car_number || entry.vehicle_number || entry.official_car_number || "");
  const fallbackDriver = featuredDriver.fallback.driver;
  const fullName = String(fallbackDriver.name || featuredDriver.name || "").toLowerCase();
  const surname = fullName.split(" ").at(-1) || "";

  return (
    String(featuredDriver.id || "") !== "" && String(id || "") === String(featuredDriver.id)
  ) || (
    fullName !== "" && name === fullName
  ) || (
    surname !== "" && name.includes(surname)
  ) || (
    number === String(fallbackDriver.number || featuredDriver.number || "")
  );
}

function findDriver(results, featuredDriver = RICKY_DRIVER) {
  return (results || []).find((entry) => driverMatches(entry, featuredDriver));
}
...
function normalizeField(liveFeed, weekendRace, featuredDriver = RICKY_DRIVER) {
  if (liveFeed?.vehicles?.length) {
    return [...liveFeed.vehicles]
      ...
      .map((entry) => ({
        ...
        isFeaturedDriver: driverMatches(entry, featuredDriver),
        isStenhouse: driverMatches(entry, featuredDriver)
      }));
  }
...
function hasStartingLineup(weekendRace, featuredDriver = RICKY_DRIVER) {
  const rows = Array.isArray(weekendRace?.results) ? weekendRace.results : [];
  const maxStartingSpot = Math.max(rows.length, 40);
  const stenhouseStart = numberOrZero(findDriver(rows, featuredDriver)?.starting_position);
...
function getRickyStageResults(weekendRace, featuredDriver = RICKY_DRIVER) {
  const rows = Array.isArray(weekendRace?.stage_results) ? weekendRace.stage_results : [];
  return rows
    .filter((row) => driverMatches(row, featuredDriver))
    .reduce((results, row) => {
...
function clampStageEndLaps(stageEnds, scheduledLaps) {
  const raceLapCap = numberOrZero(scheduledLaps);
  if (!raceLapCap || !Array.isArray(stageEnds) || !stageEnds.length) return stageEnds;

  const clamped = [];
  let previousEnd = 0;

  stageEnds.forEach((stage) => {
    const rawEnd = numberOrZero(stage.endLap);
    const cappedEnd = Math.min(rawEnd || raceLapCap, raceLapCap);
    if (cappedEnd <= previousEnd) return;
    ...
    const rawStart = numberOrZero(stage.startLap);
    if (rawStart > 0) {
      next.startLap = Math.min(Math.max(rawStart, previousEnd + 1), cappedEnd);
    }
...
function buildStagePlan(weekendRace, liveFeed, currentLap, current, featuredDriver = RICKY_DRIVER) {
  const scheduledLaps = numberOrZero(normalizedRaceLapsValue(weekendRace, liveFeed));
  const rickyStageResults = getRickyStageResults(weekendRace, featuredDriver);
  ...
  stageEnds = clampStageEndLaps(stageEnds, scheduledLaps);

  const stages = stageEnds.map((stage, index) => {
    const previousEnd = stageEnds[index - 1]?.endLap || 0;
    const rawStart = stage.startLap || previousEnd + 1;
    const startLap = Math.min(Math.max(rawStart, previousEnd + 1), stage.endLap);
    return {
      stageNumber: stage.stageNumber,
      startLap,
      endLap: stage.endLap,
...
function normalizeRace(race, weekendFeed, liveFeed, errors, featuredDriver = RICKY_DRIVER) {
  const weekendRace = weekendFeed?.weekend_race?.[0] || race || {};
  const weekendResults = weekendRace.results || [];
  const weekendStenhouse = findDriver(weekendResults, featuredDriver);
  const liveStenhouse = findDriver(liveFeed?.vehicles, featuredDriver);
  const current = liveStenhouse || weekendStenhouse || {};
  const field = normalizeField(liveFeed, weekendRace, featuredDriver);
  ...
  const qualifyingComplete = raceComplete || hasStartingLineup(weekendRace, featuredDriver);
  ...
  const lapsToGo = status === "complete"
    ...
    : Number.isFinite(parsedLapsToGo)
      ? Number.isFinite(numericScheduledLaps) && numericScheduledLaps > 0 && parsedLapsToGo > numericScheduledLaps
        ...
        : parsedLapsToGo
      : rawLapsToGo || "";
  const stagePlan = buildStagePlan(weekendRace, liveFeed, currentLap, current, featuredDriver);
  ...
  return {
    ...
    stenhouse: {
      ...
      carNumber: current.vehicle_number || current.car_number || featuredDriver.fallback.driver.number,
      sponsor: current.sponsor_name || current.sponsor || featuredDriver.fallback.driver.sponsor,
      manufacturer: current.vehicle_manufacturer || current.car_make || featuredDriver.fallback.driver.manufacturer,
...
async function getRaceDashboard(featuredDriver = RICKY_DRIVER) {
  const errors = [];
  ...
  const normalized = normalizeRace(selectedRace, weekendFeed, liveFeed, errors, featuredDriver);
  const upcomingRace = nextRace(schedule);
  normalized.nextRace = upcomingRace ? {
    raceId: upcomingRace.race_id || "",
    raceName: upcomingRace.race_name || "",
    trackName: upcomingRace.track_name || "",
    scheduled: raceTime(upcomingRace)
  } : null;

  return normalized;
}

async function getDriverDashboard(featuredDriver = RICKY_DRIVER) {
  try {
    const markdown = await getDriverMarkdown(featuredDriver);
    return parseDriverMarkdown(markdown, featuredDriver.fallback);
  } catch (error) {
    return {
      driver: featuredDriver.fallback.driver,
      season: featuredDriver.fallback.season,
      raceLog: featuredDriver.fallback.raceLog,
      annualHistory: featuredDriver.fallback.annualHistory,
      careerHighlights: featuredDriver.fallback.careerHighlights,
      error: error.message
    };
  }
}

async function buildDashboardForDriver(featuredDriver = RICKY_DRIVER) {
  const season = getSeason();
  const [driverData, currentRace] = await Promise.all([
    getDriverDashboard(featuredDriver),
    getRaceDashboard(featuredDriver)
  ]);
  ...
  return {
    generatedAt: new Date().toISOString(),
    feedSeason: season,
    feedYearNow: currentYear(),
    assets: ASSETS,
    sources: dashboardSources(currentRace, season, featuredDriver),
    warnings,
    ...driverData,
    currentRace
  };
...
export async function buildDashboard() {
  return buildDashboardForDriver(RICKY_DRIVER);
}

export async function buildMelonPatchDashboard() {
  return buildDashboardForDriver(ROSS_DRIVER);
}

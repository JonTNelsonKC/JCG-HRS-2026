# Ricky Stenhouse Jr. Superfans

A public-ready fan dashboard for Ricky Stenhouse Jr. and the No. 47 HYAK Motorsports Chevrolet.

## Local Run

```bash
npm start
```

Open `http://127.0.0.1:5173`.

The Wreckhouse archive lives at `http://127.0.0.1:5173/wreckhouse.html`, and the dashboard header links to it.

Set a different port if needed:

```bash
PORT=3000 npm start
```

## Cloudflare Pages

This repo is ready for Cloudflare Pages with GitHub.

Bindings for this project are managed through `wrangler.toml` (not the Cloudflare Bindings UI).

Project settings:

```text
Framework preset: None
Build command: exit 0
Build output directory: public
Root directory: /
```

Leave any Worker-style deploy command blank. Do not use `npx wrangler deploy` for this project. If Cloudflare asks for a deploy command anyway, use:

```bash
npm run deploy
```

Cloudflare Pages will serve the files in `public/` and automatically turn these files into API routes:

```text
functions/api/dashboard.js -> /api/dashboard
functions/api/wreckhouse.js -> /api/wreckhouse
functions/api/health.js -> /api/health
functions/api/schedule.js -> /api/schedule
functions/api/news.js -> /api/news
functions/api/f1-rankings.js -> /api/f1-rankings
```

Deploy flow:

1. Push this project to a GitHub repository.
2. In Cloudflare, go to Workers & Pages, select the Pages tab, create a Pages project, and import the GitHub repo.
3. Use the settings above and deploy.
4. After deployment, open the generated `*.pages.dev` URL.

If your build log says `Executing user deploy command: npx wrangler deploy`, you are using the wrong deploy command for Pages. Clear that field or change it to `npm run deploy`, then retry the deployment.

If your build log says `Output directory "public" not found`, Cloudflare is looking at a GitHub directory that does not contain this project. In the GitHub repo root, you should see these paths:

```text
public/index.html
public/app.js
public/styles.css
functions/api/dashboard.js
functions/api/wreckhouse.js
functions/_lib/nascar-data.js
package.json
wrangler.toml
```

If those files are inside a nested folder in GitHub, set Cloudflare's root directory to that folder. If they are missing, upload or push the full project while preserving the folder structure.

For a Cloudflare-style local preview, use:

```bash
npm run pages:dev
```

Local `npm start` and Cloudflare Pages Functions both use the same shared data module at `functions/_lib/nascar-data.js` so preview and production stay aligned.

### Shared F1 Fan Rankings

The F1 fan rankings page now reads/writes to `/api/f1-rankings`.

- Local preview (`npm start`): rankings are shared for everyone hitting that running dev server process.
- Cloudflare production: rankings are globally shared and persistent through KV binding `F1_RANKINGS` in `wrangler.toml`.
- Set `[[kv_namespaces]].id` in [wrangler.toml](/Users/joelgray/Documents/Codex/2026-04-26/build-me-a-website-that-is/wrangler.toml) to your real namespace ID once, then future deploys are push-only.

Optional environment variable:

- `F1_DELETE_PIN` (defaults to `1700`)

## Data

The shared data layer polls NASCAR public race feeds for schedule, weekend, and live leaderboard data. During live race windows, the browser refreshes every 3 seconds and the API keeps the live feed cache to roughly 2.5 seconds. It also reads the official NASCAR driver stats page through a text fallback so the dashboard still has current season and career stats when the live race feed is not active.
